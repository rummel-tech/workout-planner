...
class HealthSync {
  ...
  final int batchSize;
  HealthSync({required this.userId, String? baseUrl, this.batchSize = 100})
  ...
  Future<HealthSyncResult> perform() async {
    ...
    // On web, Platform.isIOS throws an exception, so check kIsWeb first
    if (kIsWeb) {
      // Health sync not available on web platform
      return const HealthSyncResult(inserted: 0, total: 0, errors: []);
    }
    bool granted = true;
    if (Platform.isIOS) {
